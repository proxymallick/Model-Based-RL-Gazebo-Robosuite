...
class EElab_test5(SingleArmEnv):
    """
    ...
    def __init__(
        ...
        table_full_size=(1.82, 1.22, 0.015),
        ...
        self.table_offset = np.array((0, 0, 0.92))
        self.old_dis = 0
    ...
    def staged_rewards(self):
        ...
        # ballA_pos = self.sim.data.body_xpos[self.ballA_body_id]
        gripper_site_pos = self.sim.data.site_xpos[self.robots[0].eef_site_id]
        # dist = np.linalg.norm(gripper_site_pos - ballA_pos)
        dist = np.linalg.norm(gripper_site_pos - cubeA_pos)
        r_reach = 1 - np.tanh(10.0 * dist) # + np.tanh(10.0 * self.old_dis))
        touch_sensor = self.sim.data.get_sensor('touchsensor')

        if touch_sensor != 0:
            r_reach -=(0.01*touch_sensor)
            # print('touch_sensor = ', touch_sensor)
        # grasping reward
    ...
    def _load_model(self):
        ...
        self.robots[0].robot_model.set_base_xpos([-0.4, -0.9, 0])
        ...
        if self.placement_initializer is not None:
            ...
            # self.placement_initializer.add_objects(balls)
        else:
            self.placement_initializer = UniformRandomSampler(
                name="ObjectSampler",
                mujoco_objects=cubes,
                # mujoco_objects=balls,
                x_range=[-0.2, 0.2],
                y_range=[-0.2, 0.2],
                rotation=None,
                ...
                z_offset = np.clip(random.rand(), 0.01, 0.3)
                # z_offset=0.3,
            )
    ...
    def _reset_internal(self):
        ...
        # self.sphere.set('pos', '0.01 0.01 0.01')
        ...
        if not self.deterministic_reset:
            ...
            #     self.sim.data.set_joint_qpos(obj.joints[0], np.concatenate([np.array(obj_pos), np.array(obj_quat)]))
                # self.sim.data.set_body_xpos(np.concatenate([np.array(obj_pos)]))

            # We know we're only setting a single object (the door), so specifically set its pose
            cubeA_pos, cubeA_quat, _ = object_placements[self.cubeA.name]
            cubeA_body_id = self.sim.model.body_name2id(self.cubeA.root_body)
            self.sim.model.body_pos[cubeA_body_id] = cubeA_pos
            self.sim.model.body_quat[cubeA_body_id] = cubeA_quat
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            # @sensor(modality=modality)
            # def gripper_to_ballA(obs_cache):
            #     return obs_cache["ballA_pos"] - obs_cache[f"{pf}eef_pos"] if \
            #         "ballA_pos" in obs_cache and f"{pf}eef_pos" in obs_cache else np.zeros(3)

            @sensor(modality=modality)
            def table_contect(obs_cache):
                return np.array(self.sim.data.get_sensor('touchsensor'))



            sensors = [cubeA_pos, cubeA_quat, gripper_to_cubeA]
            # sensors = [ballA_pos, gripper_to_ballA]
    ...
    def visualize(self, vis_settings):
        ...
        if vis_settings["grippers"]:
            ...
            # self._visualize_gripper_to_target(gripper=self.robots[0].gripper, target=self.ballA)
